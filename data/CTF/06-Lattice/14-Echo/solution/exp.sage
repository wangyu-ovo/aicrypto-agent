...
use_payload = b"cat flag.txt;#00000000"
got = False
while not got:
    r = remote("127.0.0.1", int(1337))
    start = time.time()
    kn = []
    ...
    for k in range(100, 140):
        ...
        for row in M:
            if (row[-1] == 0 or abs(row[-1]) == 1) and row[-2] == 1:
                print("Found")
                suffix = [row[i] + 64 for i in range(k)]
                try:
                    suffix = bytes(suffix)
                    tmp = suffix.decode()
                except:
                ...
                print("Okay")
                print(suffix)
                assert b2l(use_payload + suffix) % modulus == row[-1] % modulus
                print(use_payload + suffix)
                print(row[-1])
                r.sendlineafter("> ", b"2")
                r.sendlineafter(": ", use_payload + suffix)
                r.sendlineafter(": ", str(row[-1] % modulus))
                r.interactive()
                got = True
                break
    r.close()
